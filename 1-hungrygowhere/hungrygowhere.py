"""
This script is used to scrap hungrygowhere restaurants data and their reviews.
To start scrapping, please update binary location in get_browser function.

By default, only 10 restaurants is going to be scraped. To change this, open config.py and change the LIMIT value.
...
# Exclude this list from web scrapping
...
def get_browser(driver=None):
    if driver == "chrome":
        ...
        ### UPDATE CHROME BINARY LOCATION ###
        chrome_options.binary_location = '/Applications/Google Chrome Canary.app/Contents/MacOS/Google Chrome Canary' # MacOS

        return webdriver.Chrome(executable_path=os.path.abspath("chromedriver"), chrome_options=chrome_options)
    else:
        ### UPDATE PHANTOMJS BINARY LOCATION ###
